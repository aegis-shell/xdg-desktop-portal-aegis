...
`pam_aegis.so`'s `authenticate` hook stashes the authtok in PAM module data
...
runtime directory. The `aegis-portal-secret` component consumes and deletes
...
authentication token, `session optional pam_aegis.so` after the logind
session module, and `password optional pam_aegis.so` after the module that
