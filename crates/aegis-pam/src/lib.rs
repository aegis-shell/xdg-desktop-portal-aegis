//! `pam_aegis`: bridge the login password and the aegis secret vault.
//!
//! Installed as `auth/session/password optional pam_aegis.so`, the module
...
//! `/run/user/<uid>/aegis-pam-token` mode 0600 through the hardened
...
//! (`aegis-key-v1:<hex>`, `aegis_portal_secret::derive_token_key_in`)
...
//! `xdg-desktop-portal-aegis` consumes and deletes the token to unlock a
...
const STASH_NAME: &[u8] = b"pam_aegis_authtok\0";
...
fn token_payload(dir: Option<&Path>, authtok: &[u8]) -> TokenPayload {
    ...
    match plant_plan(dir) {
        ...
        PlantPlan::Raw => raw(),
        PlantPlan::Derived => match aegis_portal_secret::derive_token_key_in(dir, authtok) {
            Some(token) => {
                let mut token = token;
...
fn vault_dir(xdg_data_home: Option<&[u8]>, home: &[u8]) -> Option<PathBuf> {
    ...
    Some(base.join("aegis").join("secrets"))
}
...
fn rekey_vault_in_dir(dir: &Path, old: &[u8], new: &[u8]) {
    ...
    let _ = aegis_portal_secret::rekey_password_vault_in(dir, old, new);
}
...
mod tests {
    ...
    fn token_payload_derives_the_vault_key_for_password_vaults() {
        ...
        let prefix = aegis_portal_secret::PAM_TOKEN_KEY_PREFIX;
        assert!(content.starts_with(prefix.as_bytes()));
        assert_eq!(content.len(), prefix.len() + 64);
        assert_eq!(
            content.as_slice(),
            aegis_portal_secret::derive_token_key_in(&dir, b"hunter2")
