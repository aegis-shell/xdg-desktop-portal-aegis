...
fn render_desktop_entry(app_id: &str, spec: &AutostartSpec) -> String {
    ...
    entry.push_str("X-Aegis-Portal=background\n");
    entry
}
...
fn write_autostart_entry(
    ...
    let temporary = directory.join(format!(".{app_id}.desktop.aegis-{}", std::process::id()));
...
mod tests {
    ...
    fn desktop_entry_rendering_is_complete_and_conditional() {
        ...
        assert!(entry.contains("X-Aegis-Portal=background\n"));
