...
//! `$XDG_CACHE_HOME/xdg-desktop-portal-aegis` (falling back to
//! `$XDG_RUNTIME_DIR/xdg-desktop-portal-aegis` per the same spec carve-out
...
/// `$XDG_RUNTIME_DIR/xdg-desktop-portal-aegis`, else `None` (the request
...
fn cache_dir_from(
    ...
    cache
        ...
        .map(|base| PathBuf::from(base).join("xdg-desktop-portal-aegis"))
}
...
mod tests {
    ...
    fn cache_dir_prefers_cache_home_and_falls_back_to_runtime_dir() {
        ...
        assert_eq!(
            ...
            Some(PathBuf::from("/cache/xdg-desktop-portal-aegis"))
        );
        assert_eq!(
            cache_dir_from(None, Some(runtime.clone())),
            Some(PathBuf::from("/run/user/1000/xdg-desktop-portal-aegis"))
        );
        assert_eq!(
