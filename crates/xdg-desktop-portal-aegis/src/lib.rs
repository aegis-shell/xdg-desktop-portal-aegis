...
//! inhibits hold the scope's connection-scoped global idle inhibitor.
...
use aegis_portal_secret::{PromptResponse, SecretPrompter, SecretService};
...
pub fn run() -> Result<(), PortalError> {
    ...
    let secret_service = match SecretService::initialize(Arc::new(IpcSecretPrompter {
        socket: socket.clone(),
    })) {
    ...
    if let Some(service) = &secret_service {
        service.register_portal(&conn, Arc::clone(&tracker), DESKTOP_PATH)?;
    }
    ...
    // compat-only implementation inside `aegis-portal-secret`.
    // ------------------------------------------------------------------
    let compat_served = match &secret_service {
        Some(service) => match service.serve_compat(&conn) {
            Ok(()) => true,
            Err(error) => {
    ...
    let interfaces = if secret_service.is_some() {
...
mod integration_metadata_tests {
    ...
    fn activation_uses_the_private_packaged_executable() {
        assert!(
            DBUS_SERVICE
                .lines()
                .any(|line| line == "Exec=/usr/lib/xdg-desktop-portal-aegis")
        );
    }
