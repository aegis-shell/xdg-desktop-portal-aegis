...
impl RestoreStore {
    /// Load the store rooted at `data_dir` (`$XDG_DATA_HOME/aegis-portal`,
...
/// The restore store's data directory: `$XDG_DATA_HOME/aegis-portal`, with
/// the freedesktop fallback `~/.local/share/aegis-portal`.
pub(crate) fn data_dir() -> Option<PathBuf> {
    dirs::data_dir().map(|dir| dir.join("aegis-portal"))
}
