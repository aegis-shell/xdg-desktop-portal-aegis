...
//!   `lp -t <title> <file>` (direct spawn, no shell; `AEGIS_PORTAL_LP`
...
fn lp_command() -> String {
    std::env::var("AEGIS_PORTAL_LP").unwrap_or_else(|_| "lp".to_string())
}
...
pub(crate) fn print_worker(rx: mpsc::Receiver<PrintJob>, tracker: Arc<Mutex<RequestTracker>>) {
    ...
    }) = rx.recv()
    {
        ...
        if let Err(error) = std::thread::Builder::new()
            .name("aegis-portal-print-task".to_owned())
            .spawn(move || {
...
mod tests {
    ...
    fn submission_reports_the_exit_status() {
        ...
        let spool = dir.join(format!("aegis-print-submit-{}.ps", std::process::id()));
