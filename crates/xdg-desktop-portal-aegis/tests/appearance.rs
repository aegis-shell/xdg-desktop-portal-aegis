...
use aegis_portal_ipc::{AccentColor, ColorScheme, Contrast, DesktopPreferences, SettingsSnapshot};
use aegis_portal_prompter::{ConfirmResponse, PrompterResponse};
...
fn preferences_reach_the_prompter_request() {
    ...
    let _server = Server::start(&runtime_dir.join("aegis.sock"), Arc::new(LightCompositor))
